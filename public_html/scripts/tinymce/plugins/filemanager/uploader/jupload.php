...
class JUpload
{
	...
	private function generateAppletTag($str)
	{
		...
		$js_tag = isset($this->classparams['tag_jscript']) ? strval($this->classparams['tag_jscript']) : '';
		$applet_tag = isset($this->classparams['tag_applet']) ? strval($this->classparams['tag_applet']) : '';

		$str = (string)$str;
		$str = preg_replace('/' . $js_tag . '/', (string)$this->str_jsinit(), $str);
		return preg_replace('/' . $applet_tag . '/', (string)$this->str_applet(), $str);
	}
