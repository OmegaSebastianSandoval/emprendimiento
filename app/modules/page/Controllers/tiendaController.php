...
class Page_tiendaController extends Page_mainController
{
	...
	public function indexAction()
	{
		...
		$data = array();
		$data['id_tienda'] = $id;
		$data['usuario'] = $_SESSION["kt_login_user"];
		$data['fecha'] = date("Y-m-d");
		$data['hora'] = date("h:i:s");
		$tiendaclicksModel->insert($data);
		$categoriasModel = new Administracion_Model_DbTable_Categorias();
		$this->_view->categorias  = $categoriasModel->getList(" categorias_padre='0' AND categorias_estado='1' AND categorias_estado_imagen='1'", " categorias_nombre ASC ");
